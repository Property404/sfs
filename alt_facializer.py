...
subjects = ["zero", "stressed", "relaxed","negative one"]
...
def getState(capture):
    try:
        img = cv2.imread(capture).copy()
        face, rect = detect_face(img)
        label, confidence = face_recognizer.predict(face)
        label_text = subjects[label]
    except:
        return ("error_no_state", -1)
...
if __name__=="__main__":
    ...
    #load test images
    images = ["test1.jpg","test2.jpg","1.jpg","2.jpg"]
    for image in images:
        test_img1 = cv2.imread("test-data/"+image)
        ...
        #display both images
        cv2.imshow("something", cv2.resize(predicted_img1, (400, 500)))
        cv2.waitKey(0)
        cv2.destroyAllWindows()
